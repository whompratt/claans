...
with open(f"{img_path}/settings.json") as fp:
    ...
    settings["Fire Dancers"].sort()

# Set the page title and icon and set layout to "wide" to minimise margains
st.set_page_config(page_title="Fire Dancers", page_icon=":fire:")
...
if check_password():
    ...
    with st.container():
        ...
        with head_l:
            ...
            # Add a title
            st.title("Fire Dancers")

        with head_r:
            # Add logo
            st.image(claan_img)

        st.write("Welcome to the Fire Dancer Claan area! Here you can find all of the dice you have accumulated for the current fortnight, your magic items, and submit any quests that you have been working on!")
        st.write("---")
    ...
    with st.container():
        ...
        name = st.selectbox('Please tell me who you are!',
                            tuple(["Please select your name"] + settings['Fire Dancers'])
                        )
        ...
        if st.button('Submit'):
            ...
            try:
                ...
                elif quest == "Access your Claan's secret phrase using NordLayer" and answer != st.secrets["fire_phrase"]:
